...
namespace NetShip {
    ...
    using ship_vec = std::vector<Ship>;
    ...
    namespace {
        ...
        position FindTopLeftOfShip(const bool_2d_vec& shots, position hit, bool isHorizontal) {
            auto&& [posX, posY] = hit;
            dimensions dim {shots.size(), shots.at(0).size()};
            position check = isHorizontal ? position{posX, posY - 1} : position{posX - 1, posY};
            if (IsPosValid(check, dim) && shots.at(check.first).at(check.second)) {
                return FindTopLeftOfShip(shots, check, isHorizontal);
            }
            return hit;
        }

        is_horiz FindShipDir(const bool_2d_vec& shots, position hit) {
            auto&& [posX, posY] = hit;
            dimensions dim {shots.size(), shots.at(0).size()};
            // try left/right
    ...
    opt_bool_2d_vec TryShootOrSink(const bool_2d_vec& shots, position pos) {
        auto&& [posX, posY] = pos;
        if (shots.at(posX).at(posY)) return {};
        auto retHit(shots);
    ...
    pos_vec FindSunkShip(const bool_2d_vec& shots, position lastHit) {
        bool isHorizontal = FindShipDir(shots, lastHit);
        auto&& [posX, posY] = FindTopLeftOfShip(shots, lastHit, isHorizontal);
        pos_vec retVec;
        ...
        while (IsPosValid(check, dim) && shots.at(check.first).at(check.second)) {
